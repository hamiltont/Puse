...
public class LinkActivation implements LinkActivationListener {
    ...
    public void linkActivated(LinkActivationEvent arg0) {
        ...
        if (Data.open_ref_new_window == true) {
            c.svgutils.reload(c.gui.linkwindow.seperate_svg_display, clicked_link);
            c.gui.linkwindow.setVisible(true);
            ...
            if (Data.is_saved == true) {
                if (Data.incremental_model == true) {
                    ...
                    String new_code = c.svgutils.CombineStrings(c.gui.code_text_area.getText().trim(), ref_diagram_code);

                    if (new_code.equals("")) {
                        JOptionPane.showMessageDialog(c.gui.code_text_area, ref_diagram_code + "Can not found");
                    } else {
                        c.gui.code_text_area.setText(new_code);
                        c.svgutils.writeSvgToDefaultFile(new_code);
                    ...
                    c.gui.back_button.setEnabled(true);
                    ...
                    try {
                        ...
                        c.gui.code_text_area.read(new BufferedReader(new FileReader(input_file_format)), null);
                    } catch (FileNotFoundException e) {
                        JOptionPane.showMessageDialog(c.gui.code_text_area, input_file_format + "file could not found");
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    c.gui.validation_label.setText(c.svgutils.writeSvgToDefaultFile(c.gui.code_text_area.getText()));
                    File f = new File(input_file_format);
                    c.gui.filename_label.setText(f.getName().replaceFirst(".txt", ""));
                    Data.browse_file_names.push(Data.currentfile);
                }
            }//if Data.is_saved==false
            else {
                JOptionPane.showMessageDialog(c.gui.code_text_area, "Please save file before opening a diagram");
                c.gui.save_button.doClick();
            }
        }
